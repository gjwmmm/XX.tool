 ...
 * 支持自定义 M 行 N 列切割格子和自定义宽高比例
 */
PM.register({
    id: 'cutter',
    name: '切割压缩模块',
    icon: '✂️',
    description: '自定义比例裁剪，支持 M×N 矩阵动态分割，自适应控制输出体积，支持自定义输出格式。',
    color: '--warning-color',
    ...
    init: function () {
        ...
        // 设置变更时提示重新上传
        var settings = ['cutterGridM', 'cutterGridN', 'cutterRatioW', 'cutterRatioH', 'cutterFormat', 'cutterSizeInput'];
    ...
    _handleFiles: function (files) {
        ...
        files.forEach(function (file, fileIdx) {
            ...
            batchBox.innerHTML = '<div class="batch-title">📁 [ ' + targetF + ' ] (' + file.name + ') — ' + gridM + '×' + gridN + ' 切割，比例 ' + ratioW + ':' + ratioH + '</div><div class="grid-output" id="g_box_' + fileIdx + '" style="display:grid;grid-template-columns:repeat(' + Math.min(gridN, 4) + ', 1fr);gap:8px;"></div>';
            ...
            Utils.loadImageFromFile(file).then(function (img) {
                ...
                var sW = srcW / gridN;  // 每列宽度
                var sH = srcH / gridM;  // 每行高度
                ...
                for (var r = 0; r < gridM; r++) {
                    for (var c = 0; c < gridN; c++) {
                        (function (row, col, ridx) {
                            ...
                            cvs.width = outW;
                            cvs.height = outH;
                            cvs.getContext('2d').drawImage(img, srcX + col * sW, srcY + row * sH, sW, sH, 0, 0, outW, outH);
                            ...
                            Utils.compressEngine(cvs, 0.95, maxBytes, function (blob) {
                                ...
                                if (loaded === total) {
                                    ...
                                    Utils.setStatus('cutterStatus', '✨ 处理完成！共 ' + files.length + ' 张图，' + gridM + '×' + gridN + ' 切割，比例 ' + ratioW + ':' + ratioH + '，格式: ' + formatLabel);
